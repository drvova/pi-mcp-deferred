...
export function default_mcp_trust_store_path() {
    return join(getAgentDir(), 'trusted-mcp-projects.json');
}
export function create_mcp_project_trust_subject(path, hash) {
    return {
        kind: 'mcp-config',
        id: path,
        hash,
...
export function is_project_mcp_config_trusted(path, hash, trust_store_path = default_mcp_trust_store_path()) {
    const subject = create_mcp_project_trust_subject(path, hash);
    if (is_project_subject_trusted(subject, trust_store_path))
        return true;
    const legacy_entry = read_project_trust_store(trust_store_path)[path];
    return legacy_entry?.path === path && legacy_entry.hash === hash;
}
export function trust_project_mcp_config(path, hash, trust_store_path = default_mcp_trust_store_path()) {
    trust_project_subject(create_mcp_project_trust_subject(path, hash), trust_store_path);
}
