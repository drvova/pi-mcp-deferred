import { get_context_mcp_output_limits, maybe_store_context_output, } from '@spences10/pi-context';
...
export function truncate_mcp_tool_output(text, options = {}) {
    ...
    if (context_output) {
        return {
            ...
            details: {
                ...
                max_bytes,
                max_lines,
                preview_bytes: Buffer.byteLength(context_output.preview, 'utf8'),
...
function try_store_context_output(text, options) {
    try {
...
function take_first_lines(text, max_lines) {
    const lines = text.split('\n');
    if (lines.length <= max_lines)
        return text;
    return lines.slice(0, max_lines).join('\n');
}
function take_first_utf8_bytes(text, max_bytes) {
...
function write_full_output(text, tmp_dir = tmpdir()) {
