...
import { get_project_mcp_config_info, } from './config.js';
import { create_mcp_project_trust_subject, default_mcp_trust_store_path, is_project_mcp_config_trusted, } from './trust.js';
const PROJECT_MCP_CONFIG_ENV = 'MY_PI_MCP_PROJECT_CONFIG';
function create_project_mcp_trust_subject(info) {
    const server_lines = info.servers.length === 0
        ? ['- no valid server entries detected']
        : info.servers.map((server) => `- ${server.name}: ${server.summary}`);
    return {
...
export async function get_project_mcp_config_load_decision(cwd, ctx) {
    ...
    const decision = await resolve_project_trust(create_project_mcp_trust_subject(info), {
        env: process.env,
        has_ui: ctx?.hasUI,
        select: ctx?.hasUI
    ...
    return {
        include_project: decision.action === 'allow-once' ||
            decision.action === 'trust-persisted',
        metadata_trusted: decision.action === 'trust-persisted',
    };
}
//# sourceMappingURL=project-config-loader.js.map