...
import { global_mcp_config_path, project_mcp_config_path, } from './config/paths.js';
import { get_github_repos, load_mcp_policy, policy_matches, } from './config/policy.js';
import { read_config, read_config_file, write_config_file, } from './config/read-write.js';
import { parse_server, summarize_server_entry, } from './config/server-parser.js';
...
export function load_mcp_config(cwd, options = {}) {
    ...
    return Array.from(merged_names)
        .filter((name) => policy_matches(policies[name], cwd, github_repos))
        .map((name) => {
