...
import { CONTEXT_SETTINGS_PRESETS, context_settings_from_preset, get_context_capture_limits, is_context_settings_preset, load_context_settings_config, save_context_settings_config, } from '../config.js';
import { format_context_settings_status, format_days, format_max_mb, format_output_limit, format_stats, } from '../context-format.js';
...
export async function show_context_text_modal(ctx, title, text) {
...
export async function show_context_stats(ctx) {
    const scope = scope_from_context(ctx);
    const text = format_stats(get_context_store(scope).stats({ global: true }), {
        audience: 'tui',
        title: false,
    });
    if (ctx.hasUI) {
        await show_context_text_modal(ctx, 'Context sidecar stats', `\n${text}`);
    }
    else {
...
export async function show_context_settings(ctx, options = {}) {
    if (!ctx.hasUI) {
        const scope = scope_from_context(ctx);
...
export async function handle_context_settings(ctx, args) {
    ...
    if (kind === 'show' || kind === 'current') {
        const scope = scope_from_context(ctx);
        const stats = get_context_store(scope).stats(scope);
        const text = format_context_settings_status(stats, ctx.hasUI
